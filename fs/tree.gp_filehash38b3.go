...
func init() {
	gFileHashTreeGbl.cmp = GetCmpFuncFileHash("Greater")
}
...
func NewFileHashTree(cmpType string) *FileHashTree {
	p := &FileHashTree{cmp: gFileHashTreeGbl.cmp}
	if cmpType != "" {
		p.cmp = GetCmpFuncFileHash(cmpType)
	}
	return p
}
...
func (this *__FileHashTreeNodeSortSlice) Less(i, j int) (ok bool) {
	l, r := (*this)[i], (*this)[j]
	return gFileHashTreeGbl.cmp(l.FileHash, r.FileHash)
}
