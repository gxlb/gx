...
// Last update at: [Sun Oct 30 2016 17:29:32]
// Generate from:
...
func init() {
	gFileHashGbl.cmp = CreateComparerFileHash("Greater")
}

var gFileHashGbl struct {
	cmp ComparerFileHash
}
...
//new container
func NewFileHashTree(cmpType string) *FileHashTree {
	p := &FileHashTree{cmp: gFileHashGbl.cmp}
	if cmpType != "" {
		p.cmp = CreateComparerFileHash(cmpType)
	}
	return p
}
...
func (this *__FileHashTreeNodeSortSlice) Less(i, j int) (ok bool) {
	l, r := (*this)[i], (*this)[j]
	return gFileHashGbl.cmp.F(l.FileHash, r.FileHash)
}
